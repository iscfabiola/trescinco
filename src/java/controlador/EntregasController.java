...
@ManagedBean(name = "entregasController")
@SessionScoped
public class EntregasController implements Serializable {
    ...
    @EJB
    private session.EntregasFacade ejbFacade;
    ...
    public Entregas getSelected() {
        if (current == null) {
            ...
            current.setEntregasPK(new entidad.EntregasPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            current.getEntregasPK().setVenta(current.getVenta1().getId());
    ...
    public static class EntregasControllerConverter implements Converter {
        ...
        entidad.EntregasPK getKey(String value) {
            entidad.EntregasPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new entidad.EntregasPK();
            key.setId(Integer.parseInt(values[0]));
            key.setVenta(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(entidad.EntregasPK value) {
